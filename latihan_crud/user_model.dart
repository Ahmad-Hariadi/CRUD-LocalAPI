...
String ipAddress = '192.168.144.45';

Future<List<UserModel>> getAllUser() async {
  final response =
      await http.get(Uri.parse('http://${ipAddress}:8081/api/user/getUser'));
  print(response.body);
...
Future<http.Response> createUser(UserModel data) async {
  var result = await http.post(
      Uri.parse('http://${ipAddress}:8081/api/user/insertUser'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8'
      },
      body: jsonEncode(<String, dynamic>{
...
Future<http.Response> updateUser(int id, UserModel data) async {
  return await http.put(
      Uri.parse('http://${ipAddress}:8081/api/user/updateUser/${id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, dynamic>{
...
Future<http.Response> deleteUser(int id) {
  return http.delete(
      Uri.parse('http://${ipAddress}:8081/api/user/deleteUser/${id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      });
}
